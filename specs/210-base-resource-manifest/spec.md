...
- **FR-005**: `codeReference` MUST be introduced as a recognized common property in this feature, with the same "Radius framework property" status as `application`, `environment`, and `connections` (i.e. it is part of the same shared base, is optional by default, and is treated as a Radius-aware property — not just a free-form string — when present).
- **FR-006**: Manifest YAML that exists in this repository (under `pkg/resourcetypescontrib/` and `deploy/manifest/built-in-providers/`) and any out-of-tree manifest that already declares the four common properties explicitly MUST continue to validate and register without modification.
...
- **FR-009**: The semantics of each of the four common properties at deployment / runtime MUST be unchanged from today:
  ...
  - `codeReference` is treated as a Radius-aware reference to authoring source, surfaced through the same mechanism wherever resource metadata is exposed (graph, status, etc.).
...
- **FR-012**: The base resource manifest's definition (the source of truth for the shape and behavior of the four common properties) MUST live at one well-known location in the repository, shipped with Radius, and be versioned with the rest of the resource type manifest schema. Out-of-tree resource type authors MUST NOT need to fetch or co-author it themselves.
...
- **OQ-001**: [NEEDS CLARIFICATION: Should the base resource manifest be **user-facing** (approach A: the author writes a keyword in their YAML that references the base) or **non-user-facing** (approach B: the base is applied invisibly by Radius with no keyword in the author's YAML at all)? The two approaches are equally aligned with the goal of removing per-type boilerplate; they differ on whether the inheritance is visible to anyone reading a type's YAML.
  ...
  - **Approach A — user-facing / explicit inheritance**. The author's YAML contains a keyword that opts the type into the base manifest. Common properties only appear on the type if the author opts in. Authors can publish a "raw" type by omitting the keyword. Two candidate mechanisms for the keyword have been considered:
    - **A.1 — `$ref` with a Radius-owned scheme**, e.g. `$ref: "radius:base/v1#/properties"`. Reuses standard JSON-Schema vocabulary that schema-literate authors and tooling already understand; the `v1` version segment lets authors pin a base version so a later additive change to the base doesn't silently alter pinned types. Cost: requires defining the `radius:` scheme and how the CLI / control plane resolves it. *(Recommended sub-mechanism if Approach A is chosen.)*
...
- Today's behavior of `environment` being **schema-required** on every type is treated as boilerplate that this feature explicitly removes: after this feature, `environment` is optional from the author's contract, and Radius applies its existing default / scope-based resolution when the deployment omits it. (If preserving today's "environment is required" behavior is desired for backward compatibility, that is captured under FR-004 as a per-type override the author can still apply.)
...
- The introduction of `codeReference` as a recognized Radius property is part of this feature's scope; defining what `codeReference` actually points to (a path, a URL, a structured object?) is a small companion design decision that this spec leaves open, but the property name and its "common, optional, Radius-aware" status are fixed here.
...
- Defining what `codeReference` semantically points to (a URI, a path relative to a repo root, a structured `{repo, commit, path}` object). This spec only fixes the property's name, its "common Radius property" status, and its optionality.
- Promoting any property other than the four named ones (e.g. `provisioningState`, `secrets`) to the "common Radius property" set.
