...
const (
	...
	graphOutputRenderable = "renderable"
	graphOutputBoth       = "both"
)
...
the artifact to a git orphan branch instead, stored under {source-branch}/app.json.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Marshal to JSON.
		data, err := marshalGraphArtifact(output, artifact)
		if err != nil {
...
func init() {
	...
	graphBuildCmd.Flags().StringP("file", "f", ".radius/static/app.json", "Path for the output graph artifact (local file mode)")
	graphBuildCmd.Flags().StringP("output", "o", graphOutputJSON, "Output format: json (domain model), renderable (flat node/edge list), or both")
...
func marshalGraphArtifact(output string, artifact *graph.StaticGraphArtifact) ([]byte, error) {
	switch output {
	case graphOutputJSON:
		return json.MarshalIndent(artifact, "", "  ")
	case graphOutputRenderable:
		elements := graph.BuildElements(artifact)
		return json.MarshalIndent(struct {
			Elements []graph.Element `json:"elements"`
		}{Elements: elements}, "", "  ")
	case graphOutputBoth:
		both := graph.BuildGraphArtifact(artifact)
		return json.MarshalIndent(both, "", "  ")
	default:
		return nil, fmt.Errorf("invalid output %q; expected one of: %s, %s, %s", output, graphOutputJSON, graphOutputRenderable, graphOutputBoth)
	}
}
