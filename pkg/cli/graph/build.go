...
// armResourceEntry is a normalized resource item with a symbolic name.
type armResourceEntry struct {
	SymbolicName string
	Resource     armResource
}

// armResource represents a single resource in the ARM template resources map/array.
...
func BuildStaticGraph(armJSONPath, bicepPath string) (*StaticGraphArtifact, error) {
	...
	entries, err := parseResources(tmpl.Resources)
	if err != nil {
		return nil, fmt.Errorf("parsing ARM resources: %w", err)
	}

	// Build resource lookup for symbolic name → constructed resource ID.
	resourceIDs := make(map[string]string, len(entries))
	for _, entry := range entries {
		resourceIDs[entry.SymbolicName] = constructResourceID(entry.Resource)
	}

	// Build resources and connections.
	resources := make([]*corerpv20231001preview.ApplicationGraphResource, 0, len(entries))
	for _, entry := range entries {
		...
		for _, dep := range res.DependsOn {
			depID := resolveDependency(dep, resourceIDs)
			if depID == "" {
				continue
			}
...
func parseResources(raw json.RawMessage) ([]armResourceEntry, error) {
	if len(raw) == 0 {
		return nil, nil
	}

	var mapped map[string]armResource
	if err := json.Unmarshal(raw, &mapped); err == nil {
		entries := make([]armResourceEntry, 0, len(mapped))
		...
		for _, key := range keys {
			entries = append(entries, armResourceEntry{SymbolicName: key, Resource: mapped[key]})
		}
		return entries, nil
	}

	var listed []armResource
	if err := json.Unmarshal(raw, &listed); err != nil {
		return nil, err
	}

	entries := make([]armResourceEntry, 0, len(listed))
	for i, resource := range listed {
		symbolicName := resource.Name
		if symbolicName == "" {
			symbolicName = fmt.Sprintf("resource-%d", i)
		}
		entries = append(entries, armResourceEntry{SymbolicName: symbolicName, Resource: resource})
	}
	return entries, nil
}
...
func resolveDependency(dep string, resourceIDs map[string]string) string {
	trimmed := strings.TrimSpace(dep)
	if trimmed == "" {
		return ""
	}

	if depID, ok := resourceIDs[trimmed]; ok {
		return depID
	}

	if strings.HasPrefix(trimmed, "[") {
		if depID := resolveResourceIDExpression(trimmed, resourceIDs); depID != "" {
			return depID
		}
	}
