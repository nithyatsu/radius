...
func TestBuildStaticGraph_ResourcesArray(t *testing.T) {
	t.Parallel()
	...
	bicepSource := `resource storage 'Microsoft.Storage/storageAccounts@2023-05-01' = {
  name: 'storagetest01'
  location: 'eastus'
  sku: {
    name: 'Standard_LRS'
  ...
  kind: 'StorageV2'
	...
	dir := t.TempDir()
	armPath := filepath.Join(dir, "main.json")
	bicepPath := filepath.Join(dir, "main.bicep")
	...
	resource := artifact.Application.Resources[0]
	assert.Equal(t, "storagetest01", to.String(resource.Name))
	assert.Equal(t, "Microsoft.Storage/storageAccounts", to.String(resource.Type))
	assert.Equal(t, "/planes/radius/local/resourcegroups/default/providers/Microsoft.Storage/storageAccounts/storagetest01", to.String(resource.ID))
}
